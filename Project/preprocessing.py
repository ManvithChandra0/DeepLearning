...
class PreprocessData:
    def preprocess(self, dir_path):
        ...
        for i in os.listdir(dpath):
            # Exclude hidden files (those starting with a dot) and non-directories
            if not i.startswith('.') and os.path.isdir(os.path.join(dpath, i)):
        ...
        if not labels:
            raise ValueError("No labels found. Please check the directory structure.")
